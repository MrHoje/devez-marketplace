...
const DEEP_RUNTIME_KEYS = new Set(["current_phase", "active", "skill", "version", "state_revision", "source_state_revision", "receipt", "updated_at", "last_applied_draft_id"]);
const DEEP_RECORDER_KEYS = new Set(["intent_contract", "intent_review"]);
function sanitizeDeepPayload(input) {
  plainObject(input, "deep-interview input"); const payload = { ...input }, ignored = [];
  for (const key of DEEP_RUNTIME_KEYS) if (Object.hasOwn(payload, key)) { delete payload[key]; ignored.push(key); }
  if (payload.state !== undefined) { plainObject(payload.state, "deep-interview input.state"); payload.state = { ...payload.state }; for (const key of DEEP_RECORDER_KEYS) if (Object.hasOwn(payload.state, key)) { delete payload.state[key]; ignored.push(`state.${key}`); } }
  return { payload, ignored };
}
function mergeKeyed(existing, incoming, key, label) {
  if (!Array.isArray(incoming)) throw new Error(`${label} must be an array`); const rows = Array.isArray(existing) ? existing.map(row => ({ ...row })) : []; const positions = new Map(rows.map((row, index) => [row?.[key], index]));
  for (const patch of incoming) { plainObject(patch, label); const id = required(patch[key], `${label} item requires ${key}`); if (positions.has(id)) rows[positions.get(id)] = deepMerge(rows[positions.get(id)], patch); else { positions.set(id, rows.length); rows.push({ ...patch }); } }
  return rows;
}
function mergeDeepState(base, patch, reset = false) {
  const previous = base?.state && typeof base.state === "object" ? base.state : {}; const incoming = patch.state || {}; let state = reset ? {} : { ...previous };
  if (reset && previous.intent_contract) state.intent_contract = previous.intent_contract;
  for (const [key, value] of Object.entries(incoming)) {
    if (key === "rounds") state.rounds = mergeKeyed(reset ? [] : state.rounds, value, "round_key", "state.rounds");
    else if (key === "established_facts") state.established_facts = mergeKeyed(reset ? [] : state.established_facts, value, "id", "state.established_facts");
    else if (value === null) delete state[key];
    else state[key] = value && typeof value === "object" && !Array.isArray(value) ? deepMerge(state[key], value) : value;
  }
  state.rounds ||= []; state.established_facts ||= [];
  const scored = [...state.rounds].reverse().find(row => row?.status === "scored" || Number.isFinite(Number(row?.ambiguity)));
  if (scored && Number.isFinite(Number(scored.ambiguity))) state.current_ambiguity = Math.max(0, Math.min(1, Number(scored.ambiguity)));
  return { ...base, ...patch, state };
}
function answerHash(value) { return `sha256:${sha(String(value).trim())}`; }
function canonicalIntent(items, selected) {
  if (!Array.isArray(items) || !items.length || items.length > 64) throw new Error("intent contract requires 1..64 items"); const categories = new Set(["artifact", "surface", "integration", "constraint"]); const ids = new Set();
  const normalized = items.map(item => { plainObject(item, "intent item"); const category = required(item.category, "intent item category required"); const id = required(item.id, "intent item id required"); const statement = required(item.statement, "intent item statement required").trim(); if (!categories.has(category) || !String(id).startsWith(`${category}:`) || ids.has(id) || statement.length > 1000) throw new Error(`invalid intent item ${id}`); ids.add(id); return { id: String(id).trim(), category, statement }; }).sort((a, b) => a.id.localeCompare(b.id));
  const hash = answerHash(selected); return { version: 1, items: normalized, digest: sha(JSON.stringify(normalized)), confirmation_round: 0, confirmation_answer_hash: hash, confirmation_evidence: `answer_hash:${hash}` };
}
...
function stateCommand(args, cwd) {
  ...
  if (op === "handoff") {
    ...
    next.current_phase = next.current_phase || "seeded"; next.active = true; next.handoff_from = mode; if (target === "ultragoal" && has(args, "--strict")) next.intensity = "strict"; next.updated_at = now(); next.state_revision = (next.state_revision || 0) + 1;
...
function deepInterviewCommand(args, cwd) {
  const sid = sessionId(args); const op = args[0]?.startsWith("-") ? null : args.shift(); const stateFile = statePath(cwd, sid, "deep-interview"); const draftFile = path.join(stateDir(cwd, sid), "deep-interview-draft.json");
  if (["read", "clear"].includes(op)) return stateCommand(["deep-interview", op, ...args], cwd);
  if (op === "record-answer") {
    const input = jsonArg(required(flag(args, "--input"), "record-answer requires --input"), cwd, "--input"); plainObject(input, "record-answer input"); const selected = required(input.selectedOption ?? input.selected_option, "record-answer requires selectedOption"); const current = readState(stateFile, { skill: "deep-interview", version: 1, session_id: sid, state_revision: 0, state: { rounds: [], established_facts: [] } }); current.state ||= {};
    if (input.intent_contract) { const options = input.intent_contract.confirmation_options; if (!Array.isArray(options) || !options.includes(selected)) throw new Error("selected option does not confirm the intent contract"); if (current.state.intent_contract) throw new Error("intent contract is already locked"); current.state.intent_contract = canonicalIntent(input.intent_contract.items, selected); current.state.intent_contract_required = true; }
    if (input.intent_review) { if (!current.state.intent_contract) throw new Error("intent review requires a locked intent contract"); const options = input.intent_review.approval_options || []; const observed = canonicalIntent(input.intent_review.observed_items || [], selected).items; const observedIds = new Set(observed.map(item => item.id)); const removed = current.state.intent_contract.items.map(item => item.id).filter(id => !observedIds.has(id)).sort(); const approved = options.includes(selected); const substitutions = input.intent_review.supporting_substitutions || []; for (const row of substitutions) { if (!removed.includes(row.removed_id) || !Array.isArray(row.replacement_ids) || !row.replacement_ids.length || row.replacement_ids.some(id => !observedIds.has(id)) || !String(row.rationale || "").trim()) throw new Error("invalid intent substitution"); } if (approved && removed.some(id => !substitutions.some(row => row.removed_id === id))) throw new Error("approved reduction requires a substitution for every removed intent id"); current.state.intent_review = { version: 1, status: removed.length ? (approved ? "approved" : "pending") : "not_required", locked_digest: current.state.intent_contract.digest, observed_digest: sha(JSON.stringify([...observedIds].sort())), removed_locked_ids: removed, supporting_substitutions: substitutions.map(row => ({ removed_id: row.removed_id, replacement_ids: [...new Set(row.replacement_ids)].sort(), rationale: `sha256:${sha(row.rationale.trim())}` })), ...(approved && removed.length ? { approval_round: Number(input.round || 1), answer_hash: answerHash(selected), user_answer_evidence: `answer_hash:${answerHash(selected)}` } : {}) }; }
    current.current_phase = "interviewing"; current.active = true; current.updated_at = now(); current.state_revision = (current.state_revision || 0) + 1; writeState(stateFile, current); updateActive(cwd, sid, "deep-interview", current.current_phase, true); audit(cwd, sid, "deep_interview_answer_recorded", { revision: current.state_revision, intent_locked: Boolean(input.intent_contract), intent_reviewed: Boolean(input.intent_review) }); emit({ ok: true, state_revision: current.state_revision, intent_digest: current.state.intent_contract?.digest || null, raw_answer_persisted: false }, true); return;
  }
  if (["stage", "check", "apply", "discard", "write"].includes(op)) {
    if (op === "discard") { if (fs.existsSync(draftFile)) fs.unlinkSync(draftFile); emit({ ok: true, discarded: true }, true); return; }
    const current = readState(stateFile, { skill: "deep-interview", version: 1, session_id: sid, state_revision: 0, state: { rounds: [], established_facts: [] } });
    if (op === "stage") { if (fs.existsSync(draftFile)) throw new Error("a deep-interview draft already exists; apply or discard it first"); const transition = required(flag(args, "--for"), "stage requires --for"); if (!["initialize-context", "record-round", "update-facts", "merge-state"].includes(transition)) throw new Error("unsupported deep-interview transition"); const sanitized = sanitizeDeepPayload(jsonArg(required(flag(args, "--input"), "stage requires --input"), cwd, "--input")); if (Buffer.byteLength(JSON.stringify(sanitized.payload)) > 1_000_000) throw new Error("deep-interview staged input exceeds 1 MB"); if (transition === "record-round" && (!Array.isArray(sanitized.payload.state?.rounds) || sanitized.payload.state.rounds.length !== 1)) throw new Error("record-round requires exactly one state.rounds item"); if (transition === "update-facts" && (!Array.isArray(sanitized.payload.state?.established_facts) || !sanitized.payload.state.established_facts.length)) throw new Error("update-facts requires state.established_facts"); const draft = { version: 1, draft_id: `draft-${Date.now()}-${crypto.randomBytes(4).toString("hex")}`, session_id: sid, transition, staged_against_revision: current.state_revision || 0, staged_against_sha256: sha(current), payload: sanitized.payload, ignored_runtime_owned_keys: sanitized.ignored, created_at: now() }; writeJson(draftFile, draft); emit({ ok: true, ...draft, payload: undefined }, true); return; }
    if (["check", "apply"].includes(op)) { const draft = readJson(draftFile); if (!draft) throw new Error("no staged deep-interview draft"); if (draft.staged_against_revision !== (current.state_revision || 0) || draft.staged_against_sha256 !== sha(current)) { if (op === "apply") fs.unlinkSync(draftFile); throw new Error("DI_STAGE_REVISION_CONFLICT: state changed; re-stage the delta"); } const merged = mergeDeepState(current, draft.payload); if (op === "check") { emit({ ok: true, draft_id: draft.draft_id, transition: draft.transition, result_ambiguity: merged.state.current_ambiguity ?? null, state_revision: current.state_revision || 0 }, true); return; } merged.skill = "deep-interview"; merged.version = 1; merged.session_id = sid; merged.active = true; merged.current_phase = "interviewing"; merged.last_applied_draft_id = draft.draft_id; merged.updated_at = now(); merged.state_revision = (current.state_revision || 0) + 1; writeState(stateFile, merged); fs.unlinkSync(draftFile); updateActive(cwd, sid, "deep-interview", "interviewing", true); audit(cwd, sid, "deep_interview_draft_applied", { draft_id: draft.draft_id, revision: merged.state_revision }); emit({ ok: true, draft_id: draft.draft_id, state_revision: merged.state_revision, current_ambiguity: merged.state.current_ambiguity ?? null }, true); return; }
    const sanitized = sanitizeDeepPayload(jsonArg(required(flag(args, "--input"), "write requires --input"), cwd, "--input")); const merged = mergeDeepState(current, sanitized.payload, has(args, "--reset")); merged.skill = "deep-interview"; merged.version = 1; merged.session_id = sid; merged.active = true; merged.current_phase = "interviewing"; merged.updated_at = now(); merged.state_revision = (current.state_revision || 0) + 1; writeState(stateFile, merged); updateActive(cwd, sid, "deep-interview", "interviewing", true); audit(cwd, sid, "deep_interview_state_written", { revision: merged.state_revision }); emit({ ok: true, state_revision: merged.state_revision, current_ambiguity: merged.state.current_ambiguity ?? null, ignored_runtime_owned_keys: sanitized.ignored }, true); return;
  }
  if (!has(args, "--write")) throw new Error("deep-interview requires read, clear, record-answer, stage, check, apply, discard, write, or --write --stage final");
  const stage = flag(args, "--stage") || "final"; if (stage !== "final") throw new Error("deep-interview --write supports only --stage final");
  ...
  const beforeFinal = readState(stateFile, null); if (beforeFinal?.state?.intent_contract_required) { const missing = beforeFinal.state.intent_contract.items.map(item => item.id).filter(id => !String(spec).includes(id)); const review = beforeFinal.state.intent_review; if (missing.length && (review?.status !== "approved" || stable(review.removed_locked_ids) !== stable([...missing].sort()))) throw new Error(`final spec omits locked intent ids without approved review: ${missing.join(", ")}`); }
...
function ralplanSettings(cwd) {
  const file = process.env.HOJE_CONFIG_DIR ? path.join(process.env.HOJE_CONFIG_DIR, "config.json") : path.join(cwd, ".hoje", "settings.json"); const config = readJson(file, {}); const settings = config?.hoje?.ralplan || {}; const maxIterations = Number(settings.maxIterations ?? 5); const autoHandoff = settings.autoHandoff ?? "off";
  if (!Number.isInteger(maxIterations) || maxIterations < 1 || maxIterations > 20) throw new Error("hoje.ralplan.maxIterations must be an integer from 1 to 20"); if (!["off", "ultragoal", "strict"].includes(autoHandoff)) throw new Error("hoje.ralplan.autoHandoff must be off, ultragoal, or strict"); return { maxIterations, autoHandoff };
}
function validateDisposition(document, stageN, dir) {
  plainObject(document, "disposition artifact"); if (document.schema !== "ralplan.review_conflicts.v1" || document.plannerStageN !== stageN) throw new Error("disposition schema/plannerStageN mismatch"); if (!Array.isArray(document.findings) || !Array.isArray(document.dispositions)) throw new Error("disposition findings and dispositions must be arrays"); const actions = new Set(["add", "remove", "change", "clarify"]), roles = new Set(["architect", "critic"]), incompatible = (a, b) => a !== b && (a === "remove" || b === "remove") && (a === "add" || b === "add" || a === "change" || b === "change"); const indexFile = path.join(dir, "index.jsonl"); const receipts = fs.existsSync(indexFile) ? fs.readFileSync(indexFile, "utf8").trim().split(/\r?\n/).filter(Boolean).map(JSON.parse) : [];
  for (const finding of document.findings) { plainObject(finding, "disposition finding"); if (!finding.findingId || !finding.targetId || !actions.has(finding.action) || !roles.has(finding.sourceRole) || !finding.evidence) throw new Error("invalid typed disposition finding"); const source = finding.sourceReceipt; plainObject(source, "finding sourceReceipt"); if (source.stage !== finding.sourceRole || Number(source.stageN ?? source.stage_n) !== stageN || !receipts.some(row => row.stage === source.stage && row.stage_n === stageN && row.path === source.path && row.sha256 === source.sha256)) throw new Error(`untrusted disposition provenance for ${finding.findingId}`); }
  const conflicts = []; for (let i = 0; i < document.findings.length; i++) for (let j = i + 1; j < document.findings.length; j++) { const a = document.findings[i], b = document.findings[j]; if (a.targetId === b.targetId && a.sourceRole !== b.sourceRole && incompatible(a.action, b.action)) conflicts.push(`conflict:${a.targetId}:${[a.findingId, b.findingId].sort().join(":")}`); }
  const choices = new Set(["accept_architect", "accept_critic", "synthesize", "defer_user", "reject_both"]), seen = new Set(); for (const disposition of document.dispositions) { if (!conflicts.includes(disposition.conflictId) || seen.has(disposition.conflictId) || !choices.has(disposition.choice) || !String(disposition.rationale || "").trim() || !String(disposition.decisionOwner || "").trim() || !Array.isArray(disposition.affectedSections) || !disposition.affectedSections.length) throw new Error("invalid or orphan conflict disposition"); seen.add(disposition.conflictId); } if (conflicts.some(id => !seen.has(id))) throw new Error("typed review conflict remains open"); return { conflictCount: conflicts.length };
}
...
function ralplanCommand(args, cwd) {
  ...
  const stages = new Set(["planner", "architect", "critic", "disposition", "revision", "post-interview", "adr", "final"]);
  ...
  const settings = ralplanSettings(cwd), indexFile = path.join(dir, "index.jsonl"), indexRows = fs.existsSync(indexFile) ? fs.readFileSync(indexFile, "utf8").trim().split(/\r?\n/).filter(Boolean).map(JSON.parse) : []; const normalizedArtifact = String(artifact).endsWith("\n") ? String(artifact) : `${artifact}\n`;
  if (fs.existsSync(artifactPath)) { const existingDigest = sha(fs.readFileSync(artifactPath)); const existing = indexRows.find(row => row.stage === stage && row.stage_n === stageN && row.path === artifactPath && row.sha256 === existingDigest); if (existing && existingDigest === sha(normalizedArtifact)) { emit({ ...existing, deduped: true }, has(args, "--json")); return; } throw new Error(`ralplan stage artifact already exists with different or unledgered content: ${artifactPath}`); }
  if (["planner", "revision"].includes(stage)) { const ledgerOpeners = indexRows.filter(row => ["planner", "revision"].includes(row.stage)).length, diskOpeners = fs.existsSync(dir) ? fs.readdirSync(dir).filter(name => /^stage-\d+-(planner|revision)\.md$/.test(name)).length : 0; if (Math.max(ledgerOpeners, diskOpeners) >= settings.maxIterations) { process.stdout.write("PLANNING-STUCK\n"); process.exitCode = 3; return; } }
  let disposition = null; if (stage === "disposition") { try { disposition = JSON.parse(String(artifact).replace(/^```(?:json)?\s*\n|\n```\s*$/g, "")); } catch (error) { throw new Error(`disposition artifact must be JSON: ${error.message}`); } disposition = validateDisposition(disposition, stageN, dir); }
  atomicWrite(artifactPath, normalizedArtifact); const digest = sha(fs.readFileSync(artifactPath));
  const receipt = { run_id: runId, stage, stage_n: stageN, path: artifactPath, sha256: digest, created_at: now(), ...(disposition || {}) };
  for (const key of ["planner-id", "planner-resumable", "architect-id", "architect-resumable", "critic-id", "critic-resumable", "lane-verdict", "fallback-reason", "fallback-attempted-id", "fallback-stage-n", "fallback-receipt-path"]) { const value = flag(args, `--${key}`); if (value !== undefined) receipt[key.replaceAll("-", "_")] = value; }
  if (stage === "final") receipt.auto_handoff = { configuredTarget: settings.autoHandoff, effectiveTarget: settings.autoHandoff, degradationReason: null };
...
function validateGate(gate, cwd, intensity = "standard", terminal = false) {
  ...
  const cohort = iteration.reviewCohort; plainObject(cohort, "iteration.reviewCohort"); if (!Number.isInteger(cohort.reviewGeneration) || cohort.reviewGeneration < 1 || cohort.joined !== true || !/^sha256:[a-f0-9]{64}$/i.test(cohort.sourceHash || "")) throw new Error("iteration.reviewCohort requires a positive generation, sourceHash, and joined true"); plainObject(cohort.lanes, "iteration.reviewCohort.lanes"); for (const [name, expected] of [["cleaner", "passed"], ["architect", "CLEAR"], ["qa", "passed"]]) { const lane = cohort.lanes[name]; plainObject(lane, `iteration.reviewCohort.lanes.${name}`); if (lane.status !== expected || lane.sourceHash !== cohort.sourceHash) throw new Error(`review cohort ${name} lane is not clean or source-bound`); nonempty(lane.evidence, `reviewCohort.${name}.evidence`); emptyBlockers(lane.blockers, `reviewCohort.${name}.blockers`); }
  if (terminal) { const critic = gate.criticReview; plainObject(critic, "criticReview"); if (critic.verdict !== "OKAY") throw new Error("criticReview.verdict must be OKAY"); nonempty(critic.evidence, "criticReview.evidence"); emptyBlockers(critic.blockers, "criticReview.blockers"); if (critic.sourceHash !== undefined && critic.sourceHash !== cohort.sourceHash) throw new Error("criticReview.sourceHash must match the review cohort"); }
}
...
function worktreePaths(cwd) { const result = spawnSync("git", ["status", "--short"], { cwd, encoding: "utf8", windowsHide: true }); if (result.status !== 0) return []; return [...new Set(result.stdout.split(/\r?\n/).filter(Boolean).map(line => line.slice(3).trim()).filter(Boolean))].sort(); }
function validateDeferredGate(gate, batch, goal, cwd) {
  ...
  d.kind ??= "validation-batch-deferred"; d.batchId ??= batch.batchId; d.memberGoalId ??= goal.id; if (d.kind !== "validation-batch-deferred" || d.batchId !== batch.batchId || d.memberGoalId !== goal.id) throw new Error("deferredToBatch identity does not match the validation batch member");
  for (const forbidden of ["architectReview", "executorQa", "validationBatchClose"]) if (Object.hasOwn(d, forbidden)) throw new Error(`deferredToBatch cannot contain ${forbidden}`);
  const targeted = d.targetedVerification; plainObject(targeted, "deferredToBatch.targetedVerification"); if (targeted.status !== "passed") throw new Error("deferredToBatch.targetedVerification.status must be passed"); nonempty(targeted.evidence, "deferredToBatch.targetedVerification.evidence"); stringArray(targeted.commands, "deferredToBatch.targetedVerification.commands"); targeted.blockers ??= []; emptyBlockers(targeted.blockers, "deferredToBatch.targetedVerification.blockers");
  const laneMap = { aiSlopCleaner: "cleaner", iteration: "iteration" }; const actual = ["targetedVerification", ...Object.entries(laneMap).filter(([, key]) => d[key] !== undefined).map(([lane]) => lane)]; if (d.ranLanes === undefined) { if (actual.length > 1) d.ranLanes = actual; } else if (!Array.isArray(d.ranLanes) || stable([...new Set(d.ranLanes)].sort()) !== stable([...actual].sort()) || d.ranLanes.some(lane => ["architectReview", "executorQa"].includes(lane))) throw new Error("deferredToBatch.ranLanes must exactly declare supplied lanes");
  if (d.cleaner) { if (d.cleaner.status !== "passed") throw new Error("deferredToBatch.cleaner.status must be passed"); nonempty(d.cleaner.evidence, "deferredToBatch.cleaner.evidence"); d.cleaner.blockers ??= []; emptyBlockers(d.cleaner.blockers, "deferredToBatch.cleaner.blockers"); }
  if (d.iteration) { if (d.iteration.status !== "passed" || d.iteration.fullRerun !== true) throw new Error("deferredToBatch.iteration must be passed with fullRerun true"); nonempty(d.iteration.evidence, "deferredToBatch.iteration.evidence"); stringArray(d.iteration.rerunCommands, "deferredToBatch.iteration.rerunCommands"); d.iteration.blockers ??= []; emptyBlockers(d.iteration.blockers, "deferredToBatch.iteration.blockers"); }
  d.changeSet ??= { memberGoalId: goal.id, cumulativeFromBase: true, paths: worktreePaths(cwd) }; plainObject(d.changeSet, "deferredToBatch.changeSet"); d.changeSet.memberGoalId ??= goal.id; if (d.changeSet.memberGoalId !== goal.id || d.changeSet.cumulativeFromBase !== true || !Array.isArray(d.changeSet.paths)) throw new Error("deferredToBatch.changeSet must be cumulative and bound to the member"); d.changeSet.changeSetHash ??= sha({ memberGoalId: goal.id, cumulativeFromBase: true, paths: d.changeSet.paths });
  return { batchId: batch.batchId, metadataHash: batch.metadataHash, changeSetHash: d.changeSet.changeSetHash, changeSetPaths: [...d.changeSet.paths] };
}
function validateBatchClose(gate, batch, plan, cwd) {
  const close = gate.validationBatchClose; plainObject(close, "validationBatchClose"); close.kind ??= "validation-batch-close"; close.schemaVersion ??= 1; close.batchId ??= batch.batchId; close.finalGoalId ??= batch.finalGoalId; close.memberIds ??= [...batch.memberIds]; close.memberReceiptIds ??= {}; close.memberMetadataHashes ??= {}; close.memberChangeSetHashes ??= {};
  if (close.kind !== "validation-batch-close" || close.batchId !== batch.batchId || close.finalGoalId !== batch.finalGoalId) throw new Error("validationBatchClose identity does not match the final member"); if (stable(close.memberIds) !== stable(batch.memberIds)) throw new Error("validationBatchClose.memberIds must exactly match the batch");
  const allPaths = new Set();
  for (const id of batch.memberIds.filter(id => id !== batch.finalGoalId)) { const member = plan.goals.find(goal => goal.id === id); if (member?.status !== "complete" || !member.completionVerification?.deferredToBatch) throw new Error(`validation batch member ${id} is not freshly deferred-complete`); const proof = member.completionVerification.deferredToBatch; close.memberReceiptIds[id] ??= member.completionVerification.receiptId; close.memberMetadataHashes[id] ??= proof.metadataHash; close.memberChangeSetHashes[id] ??= proof.changeSetHash; if (close.memberReceiptIds[id] !== member.completionVerification.receiptId || close.memberMetadataHashes[id] !== proof.metadataHash || close.memberChangeSetHashes[id] !== proof.changeSetHash) throw new Error(`validationBatchClose proof mismatch for ${id}`); for (const item of proof.changeSetPaths) allPaths.add(typeof item === "string" ? item : item.path); }
  for (const item of worktreePaths(cwd)) allPaths.add(item); close.unionChangeSet ??= { source: "validation-batch", cumulativeFromBase: true, paths: [...allPaths].sort() }; close.unionChangeSet.source ??= "validation-batch"; if (close.unionChangeSet.cumulativeFromBase !== true || close.unionChangeSet.source !== "validation-batch" || !Array.isArray(close.unionChangeSet.paths)) throw new Error("validationBatchClose.unionChangeSet is invalid"); for (const item of allPaths) if (!close.unionChangeSet.paths.some(row => (typeof row === "string" ? row : row.path) === item)) throw new Error(`validationBatchClose union is missing ${item}`);
  nonempty(close.coverageEvidence ?? close.evidence, "validationBatchClose.coverageEvidence");
}
...
function requiredGoals(plan) { return plan.goals.filter(goal => goal.status !== "superseded"); }
function gateForGoal(plan, goal, gate, cwd) { const explicit = plan.validationBatches.find(item => item.memberIds.includes(goal.id)); const required = requiredGoals(plan); const finalGoal = required.at(-1); if (explicit && goal.id !== explicit.finalGoalId) return { kind: "deferred", proof: validateDeferredGate(gate, explicit, goal, cwd) }; if (!explicit && plan.hojeGoalMode === "aggregate" && goal.id !== finalGoal?.id) { const boundary = { batchId: "aggregate-default", memberIds: required.map(item => item.id), finalGoalId: finalGoal.id, metadataHash: sha(required.map(item => item.id)) }; return { kind: "deferred", proof: validateDeferredGate(gate, boundary, goal, cwd) }; } validateGate(gate, cwd, explicit ? "strict" : plan.intensity === "light" ? "standard" : plan.intensity, goal.id === finalGoal?.id); if (explicit) validateBatchClose(gate, explicit, plan, cwd); return { kind: "full", proof: null }; }
...
function ultragoalCommand(args, cwd) {
  ...
  if (op === "quality-gate") { if (args[0] !== "validate") throw new Error("quality-gate requires validate"); const goalId = flag(args, "--goal-id") || currentGoal(plan, true)?.id; const goal = plan.goals.find(item => item.id === goalId); if (!goal) throw new Error("quality-gate validate requires a known --goal-id or current goal"); const gate = jsonArg(required(flag(args, "--quality-gate-json"), "quality-gate validate requires --quality-gate-json"), cwd, "--quality-gate-json"); try { const result = plan.intensity === "light" && plan.goals.length === 1 ? (validateLightGate(gate), { kind: "light" }) : gateForGoal(plan, goal, gate, cwd); emit({ ok: true, goal_id: goal.id, gate_kind: result.kind, quality_gate: gate, diagnostics: [] }, true); } catch (error) { emit({ ok: false, goal_id: goal.id, diagnostics: [{ path: "qualityGate", code: "QUALITY_GATE_INVALID", message: error.message }] }, true); process.exitCode = 1; } return; }
  ...
  if (op === "checkpoint") {
    ...
    if (status === "complete") { if (goal.status !== "active") throw new Error(`goal ${goalId} must be active before completion`); if (plan.pipelineOverlap && !["joined", "rebaselined"].includes(plan.pipelineOverlap.status)) throw new Error("cannot complete while pipeline overlap is open or quarantined"); const gate = jsonArg(required(flag(args, "--quality-gate-json"), "complete checkpoint requires --quality-gate-json"), cwd, "--quality-gate-json"); let result; if (plan.intensity === "light" && plan.goals.length === 1) { validateLightGate(gate); result = { proof: null }; } else result = gateForGoal(plan, goal, gate, cwd); const nonOkay = readLedger(cwd, sid).filter(row => row.event === "critic_verdict" && row.verdict !== "OKAY").length; const override = readLedger(cwd, sid).some(row => row.event === "critic_gate_override"); if (goal.id === requiredGoals(plan).at(-1)?.id && nonOkay >= 5 && !override) throw new Error("terminal critic retry ceiling reached; record-critic-gate-override is required"); const receipt = appendLedger(cwd, sid, "goal_checkpointed", { goalId, status, evidence, intensity: plan.intensity, qualityGateHash: sha(gate), qualityGateJson: gate }); goal.completionVerification = { receiptId: receipt.id, qualityGateHash: sha(gate), checkpointedAt: receipt.timestamp, ...(result.proof ? { deferredToBatch: result.proof } : {}) }; }
  ...
  if (op === "record-review-blockers") { const title = required(flag(args, "--title"), "record-review-blockers requires --title"); const objective = required(flag(args, "--objective"), "record-review-blockers requires --objective").trim(); const blockedGoalId = flag(args, "--goal-id") || currentGoal(plan, true)?.id || null; const existing = plan.goals.find(goal => goal.status === "review_blocked" && goal.objective.trim() === objective && goal.steering?.blockedGoalId === blockedGoalId); if (existing) { emit({ ok: true, goal_id: existing.id, deduped: true }, true); return; } const descendants = plan.goals.filter(goal => goal.status === "review_blocked" && goal.steering?.blockedGoalId === blockedGoalId); if (descendants.length >= 3) throw new Error("review_blocker_recursion_cap: resolve or escalate existing blockers"); const id = `G${String(Math.max(0, ...plan.goals.map(g => Number(g.id.slice(1)) || 0)) + 1).padStart(3, "0")}`; plan.goals.push({ id, title, objective, status: "review_blocked", evidence: flag(args, "--evidence") || "review blocker", steering: { kind: "review_blocker", blockedGoalId }, createdAt: now(), updatedAt: now() }); writePlan(cwd, sid, plan); appendLedger(cwd, sid, "review_blockers_recorded", { goalId: id, blockedGoalId, evidence: flag(args, "--evidence") || null }); emit({ ok: true, goal_id: id, deduped: false }, true); return; }
  ...
  if (op === "record-critic-verdict") { const terminus = required(flag(args, "--terminus"), "record-critic-verdict requires --terminus"), verdict = required(flag(args, "--verdict"), "record-critic-verdict requires --verdict"); if (!["pause", "completion"].includes(terminus) || !["OKAY", "ITERATE", "REJECT"].includes(verdict)) throw new Error("invalid critic terminus or verdict"); const blockers = flag(args, "--blockers-json") ? jsonArg(flag(args, "--blockers-json"), cwd, "--blockers-json") : []; blockersArray(blockers, "critic blockers"); if (verdict === "OKAY" && blockers.length) throw new Error("an OKAY critic verdict requires empty blockers"); const row = appendLedger(cwd, sid, "critic_verdict", { terminus, verdict, evidence: required(flag(args, "--evidence"), "critic verdict evidence required"), blockers, classificationEventId: flag(args, "--classification-event-id") || null, planGeneration: sha(plan.goals.map(goal => [goal.id, goal.status, goal.updatedAt])) }); emit(row, true); return; }
  if (op === "record-critic-gate-override") { const row = appendLedger(cwd, sid, "critic_gate_override", { evidence: required(flag(args, "--evidence"), "critic gate override requires evidence"), planGeneration: sha(plan.goals.map(goal => [goal.id, goal.status, goal.updatedAt])) }); emit(row, true); return; }
  if (op === "pause") { const rows = readLedger(cwd, sid), classification = [...rows].reverse().find(row => row.event === "blocker_classified"), verdict = [...rows].reverse().find(row => row.event === "critic_verdict" && row.terminus === "pause"); const generation = sha(plan.goals.map(goal => [goal.id, goal.status, goal.updatedAt])); if (!classification || classification.classification !== "human_blocked" || !verdict || verdict.verdict !== "OKAY" || verdict.classificationEventId !== classification.id || verdict.timestamp <= classification.timestamp || verdict.planGeneration !== generation || verdict.blockers.length) throw new Error("pause requires a fresh human_blocked classification and bound clean terminal critic verdict"); const nonOkay = rows.filter(row => row.event === "critic_verdict" && row.verdict !== "OKAY").length; if (nonOkay >= 5 && !rows.some(row => row.event === "critic_gate_override")) throw new Error("terminal critic retry ceiling reached; override required"); updateActive(cwd, sid, "ultragoal", "paused", false); const stateFile = statePath(cwd, sid, "ultragoal"), state = readState(stateFile, {}); state.current_phase = "paused"; state.active = false; state.updated_at = now(); state.state_revision = (state.state_revision || 0) + 1; writeState(stateFile, state); emit(appendLedger(cwd, sid, "run_paused", { classificationEventId: classification.id, criticVerdictId: verdict.id, planGeneration: generation }), true); return; }
...
function help() { emit(`Hoje native runtime ${runtime.runtimeVersion}\n\nCommands:\n  hoje runtime version|doctor\n  hoje state [deep-interview|ralplan|ultragoal] read|write|clear|handoff|doctor\n  hoje deep-interview read|clear|record-answer|stage|check|apply|discard|write\n  hoje deep-interview --write --stage final --slug <slug> --spec <markdown-or-path> [--deliberate]\n  hoje ralplan <task> | --write --stage planner|architect|critic|disposition|revision|post-interview|adr|final ...\n  hoje ultragoal status|create-goals|complete-goals|checkpoint|quality-gate validate|pause|review|steer|record-review-blockers|classify-blocker|record-critic-verdict|record-critic-gate-override|start-pipeline-overlap|join-pipeline-overlap|rebaseline-pipeline-overlap`); }
