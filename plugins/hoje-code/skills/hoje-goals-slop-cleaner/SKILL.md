...
- Recursion guard: you are already inside an Hoje Goals workflow. Do NOT spawn nested `ralplan`, `strict mode`, `deep-interview`, or `ultragoal` workflows. Broad, ambiguous, cross-layer, or architectural findings are handed to the leader as review blockers, not resolved here.
...
Recursion Guard: [confirmed no nested ralplan/strict mode/deep-interview/ultragoal spawned; broad findings handed to leader]
