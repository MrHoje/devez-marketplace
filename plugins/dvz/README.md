...
| 커맨드 | 하는 일 | 인자 |
|---|---|---|
| `/dvz:send` | 대화 맥락을 정리해 **기존 세션**에 자립적 지시문 전달 | `<세션이름> <시킬 일>` |
| `/dvz:child` | 대화 맥락을 브리핑으로 정리해 **새 자식 세션**을 만들어 위임 | `<위임할 작업 설명>` |
| `/dvz:pick` | **다른 세션**의 대화/작업 내용을 읽어와 현재 세션이 숙지 | `<세션이름>에서 …읽어/merge해/요약해` |
...
```
/plugin install dvz@devez-marketplace
```
...
| 경로 | 역할 |
|---|---|
| `commands/send.md` | `/dvz:send` — 기존 세션에 지시 전달 |
| `commands/child.md` | `/dvz:child` — 자식 세션 생성·위임 |
| `commands/pick.md` | `/dvz:pick` — 다른 세션 내용 읽어와 숙지 |
